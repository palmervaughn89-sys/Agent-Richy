    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full rounded-lg border border-navy-600 bg-navy-700 py-2 text-sm text-white
                      focus:ring-2 focus:ring-gold-500 outline-none transition-shadow
  ...
  return (
    <div>
      ...
      <select
        ...
        className="w-full rounded-lg border border-navy-600 bg-navy-700 px-3 py-2 text-sm text-white
                   focus:ring-2 focus:ring-gold-500 outline-none transition-shadow"
      >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-20 md:pb-6 space-y-6">
        ...
        {saved && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed top-4 right-4 z-50 bg-green-600 text-white text-xs font-semibold px-3 py-1.5 rounded-lg shadow"
          >
            ✓ Saved
          </motion.div>
        )}
        ...
        {/* ── Personal Info ─────────────────────────────────────────── */}
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-navy-600 bg-navy-700 px-3 py-2 text-sm text-white focus:ring-2 focus:ring-gold-500 outline-none"
              />
            </div>
        ...
        {/* ── Income & Expenses ─────────────────────────────────────── */}
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          <h3 className="text-sm font-semibold text-white mb-4">💰 Income & Expenses</h3>
          ...
          <div className={`rounded-lg p-3 ${surplus >= 0 ? 'bg-green-900/20 border border-green-800/40' : 'bg-red-900/20 border border-red-800/40'}`}>
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-300">Monthly Surplus</span>
              <span className={`text-sm font-bold ${surplus >= 0 ? 'text-green-400' : 'text-red-400'}`}>{fmt(surplus)}</span>
            </div>
          </div>
        </section>
        ...
        {/* ── Savings & Emergency ───────────────────────────────────── */}
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          <h3 className="text-sm font-semibold text-white mb-4">🏦 Savings</h3>
          ...
          {(profile?.monthly_expenses ?? 0) > 0 && (
            <p className="text-xs text-gray-500 mt-3">
              Emergency fund covers <span className="font-bold text-gold-400">
        ...
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          ...
          {Object.entries(profile?.debts ?? {}).length > 0 ? (
            <div className="space-y-2 mb-4">
              {Object.entries(profile?.debts ?? {}).map(([name, balance]) => (
                <div key={name} className="flex items-center justify-between rounded-lg bg-navy-700/50 border border-navy-600 px-3 py-2">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-gray-200">{fmt(balance)}</span>
                    <button onClick={() => removeDebt(name)} className="text-gray-500 hover:text-red-400 text-xs">✕</button>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-gray-500 mb-4">No debts recorded. 🎉</p>
          )}
          ...
          <div className="flex gap-2 flex-wrap">
            <input
              ...
              className="flex-1 min-w-[120px] rounded-lg bg-navy-700 border border-navy-600 px-3 py-2 text-sm text-white placeholder-gray-500 outline-none focus:ring-2 focus:ring-gold-500"
            />
            <input
              type="number"
              placeholder="Balance"
            ...
            <input
              ...
              className="w-20 rounded-lg bg-navy-700 border border-navy-600 px-3 py-2 text-sm text-white placeholder-gray-500 outline-none focus:ring-2 focus:ring-gold-500"
            />
            <button
              onClick={addDebt}
        ...
        {/* ── Credit Score ──────────────────────────────────────────── */}
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          <h3 className="text-sm font-semibold text-white mb-4">📊 Credit Score</h3>
            ...
            <div className="mt-3">
              <div className="h-3 rounded-full bg-navy-700 overflow-hidden">
                <motion.div
                  ...
                  className={`h-full rounded-full ${
                    profile.credit_score >= 740 ? 'bg-green-500'
                    : profile.credit_score >= 670 ? 'bg-gold-500'
              ...
              <div className="flex justify-between text-[10px] text-gray-500 mt-1">
                ...
                <span className={`font-bold ${
                  profile.credit_score >= 740 ? 'text-green-400'
                  : profile.credit_score >= 670 ? 'text-gold-400'
        ...
        {/* ── Goals ─────────────────────────────────────────────────── */}
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          <h3 className="text-sm font-semibold text-white mb-4">🎯 Financial Goals</h3>

          {(profile?.goals ?? []).length > 0 ? (
            <div className="flex flex-wrap gap-2 mb-4">
              {profile!.goals.map((g) => (
                <span key={g} className="inline-flex items-center gap-1 bg-navy-700 border border-navy-600 rounded-full px-3 py-1 text-xs text-gray-200">
                  {g}
                  <button onClick={() => removeGoal(g)} className="text-gray-500 hover:text-red-400 ml-1">✕</button>
                </span>
              ))}
            </div>
          ) : (
            <p className="text-xs text-gray-500 mb-4">No goals yet — add some below.</p>
          )}
            ...
            {FINANCIAL_GOALS
              ...
              .map((g) => (
                <button
                  ...
                  className="px-2 py-1 rounded-full bg-navy-700/50 border border-navy-600/50 text-[10px] text-gray-400 hover:text-gold-400 hover:border-gold-500/50 transition-colors"
                >
                  + {g}
                </button>
              ))}
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addGoal} className="px-4 py-2 rounded-lg bg-gold-500 hover:bg-gold-600 text-white text-sm font-semibold transition-colors">
              Add
            </button>
          </div>
        </section>
        ...
        {/* ── Risk Tolerance ────────────────────────────────────────── */}
        <section className="rounded-xl bg-navy-800 border border-navy-700 p-5">
          <h3 className="text-sm font-semibold text-white mb-4">⚖️ Risk Tolerance</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {RISK_TOLERANCE_OPTIONS.map((opt) => (
              ...
              >
                <p className={`text-sm font-semibold ${profile?.risk_tolerance === opt.value ? 'text-gold-400' : 'text-white'}`}>
                  {opt.label}
                </p>
                <p className="text-[10px] text-gray-500 mt-1">{opt.description}</p>
              </button>
            ))}
          </div>
        </section>
